...
import { formatCurrency, formatDate, generateUniqueAccountNumber } from '../../utils/helpers';
...
describe('AccountService', () => {
    ...
    describe('createAccount', () => {
        it('should create an account successfully', async () => {
            ...
            const result = await AccountService.createAccount(payload, mockTransaction);
            expect(AccountModel.createAccount).toHaveBeenCalledWith(payload, mockTransaction);
        ...
        it('should throw an InternalError if account creation fails', async () => {
            ...
            await expect(AccountService.createAccount({
                ...
                status: AccountStatus.ACTIVE,
                balance: 0,
            }, mockTransaction)).rejects.toThrow(InternalError);
        });
    });
    ...
    describe('updateBalance', () => {
        it('should update the account balance', async () => {
            ...
            await accountService.updateBalance(amount, PaymentType.DEBIT, mockTransaction);

            expect(AccountModel.updateBalance).toHaveBeenCalledWith(account_number, amount, PaymentType.DEBIT, mockTransaction);
        });
    });
    ...
    describe('notification', () => {
        it('should send a transaction notification email', async () => {
            ...
            const accountService = new AccountService(account_number);
            await accountService.notification(amount, reference_id, description, PaymentType.CREDIT, mockTransaction);
        ...
        it('should throw an InternalError if notification fails', async () => {
            ...
            await expect(
                accountService.notification(amount, reference_id, description, PaymentType.CREDIT, mockTransaction)
