...
describe('UserService Tests', () => {
    ...
    describe('createUser', () => {
        it('should create a new user with an account and send verification email', async () => {
            ...
            (generateUniqueAccountNumber as jest.Mock).mockResolvedValue(accountDto.account_number);
            (EncryptionService.hash as jest.Mock).mockImplementation((password) => {
                console.log("Hashing password:", password);  // Log original password
                return Promise.resolve(hashedPassword);
            });
            (UserModel.createUser as jest.Mock).mockImplementation(async (user) => {
                console.log("Received user in UserModel.createUser:", user);  // Debugging log
                return { ...userDto, password: user.password };
            });
            (AccountService.createAccount as jest.Mock).mockResolvedValue(accountDto);
            ...
            expect(sendEmail).toHaveBeenCalledWith(userDto.email, 'Verification Code', expect.any(String));
        ...
        it('should throw an InternalError if user creation fails', async () => {
            ...
            await expect(UserService.createUser(userDto)).rejects.toThrow(InternalError);
        });
    });
    describe('sendVerificationEmail', () => {
        it('should send a verification email with an OTP code', async () => {
            ...
            await UserService.sendVerificationEmail(userDto.email, firstName, subject);

            expect(generateOtp).toHaveBeenCalled();
            expect(CacheService.setCache).toHaveBeenCalledWith(userDto.email, verificationCode);
        ...
        it('should throw an InternalError if sending the email fails', async () => {
            ...
            await expect(UserService.sendVerificationEmail(userDto.email, 'John', 'Verification Code')).rejects.toThrow(InternalError);
        });
    });
    ...
    describe('getUserById', () => {
        it('should return user and associated account details', async () => {
            ...
            (UserModel.getUserByIdentifier as jest.Mock).mockResolvedValue(userDto);
            (AccountModel.getAccountByUserId as jest.Mock).mockResolvedValue(accountData);
            (omitValue as jest.Mock).mockReturnValue(userDto);

            const result = await UserService.getUserById(userDto.id as string);

            expect(UserModel.getUserByIdentifier).toHaveBeenCalledWith({ id: userDto.id });
            expect(AccountModel.getAccountByUserId).toHaveBeenCalledWith(userDto.id);
            expect(result).toEqual({
                user: expect.objectContaining({
                    first_name: userDto.first_name,
                    last_name: userDto.last_name,
        ...
        it('should throw an InternalError if user fetch fails', async () => {
            (UserModel.getUserByIdentifier as jest.Mock).mockRejectedValue(new Error('DB error'));

            await expect(UserService.getUserById(userDto.id as string)).rejects.toThrow(InternalError);
        });
    });
    ...
    describe('verifyUser', () => {
        it('should verify the user and update account status', async () => {
            ...
            expect(UserModel.getUserByIdentifier).toHaveBeenCalledWith({ email: userDto.email });
            expect(UserModel.updateUserById).toHaveBeenCalledWith(userDto.id, { email_verified: true });
            expect(AccountModel.updateAccountByIdOrUserId).toHaveBeenCalledWith(
                { user_id: userDto.id },
                { status: AccountStatus.ACTIVE }
            );
            expect(CacheService.invalidateCache).toHaveBeenCalledWith(userDto.email);
        });

        it('should throw an InternalError if user verification fails', async () => {
            (UserModel.getUserByIdentifier as jest.Mock).mockRejectedValue(new Error('DB error'));

            await expect(UserService.verifyUser(userDto.email)).rejects.toThrow(InternalError);
        });
    });
});
